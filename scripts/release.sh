...
Examples:
  scripts/release.sh 0.0.50
  scripts/release.sh patch

Options:
  --yes, -y       Push without the confirmation prompt.
  --skip-checks   Skip the local lint/build/test pre-flight.
EOF
  exit 1
}
...
echo -e "${CYAN}Releasing @camera.ui/cli: $cur -> $NEW (tag $TAG)${NC}"

if [ "$SKIP_CHECKS" = false ]; then
  echo -e "${YELLOW}Running lint, build and tests...${NC}"
  npm run lint
  npm run build
  npm test
fi
